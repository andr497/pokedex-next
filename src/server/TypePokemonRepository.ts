...
export const findPokemonTypes = async (
    ...
    const typeDetails: TypeRelations[] = await Promise.all(
        types.map(async ({ type }) => {
            let pokemonTypes: TypeDetail = await getTypeDetail(type.name).then(
                (res) => res
            );
...
function _processTypes(typeDetails: TypeRelations[]): TypeDefenses {
    ...
    typeDefenses.weak = typeDefenses.weak.filter(
        (value) => !typeDefenses.immune.includes(value)
    );
    typeDefenses.resistant = typeDefenses.resistant.filter(
        (value) => !typeDefenses.immune.includes(value)
    );

    let normalTypes = typeDefenses.weak.filter((value) =>
        typeDefenses.resistant.includes(value)
    );

    typeDefenses.weak = typeDefenses.weak.filter(
        (value) => !normalTypes.includes(value)
    );
    typeDefenses.resistant = typeDefenses.resistant.filter(
        (value) => !normalTypes.includes(value)
    );

    let tempWeak: string[] = Object.keys(
        findDuplicateElements(typeDefenses.weak)
    );
    typeDefenses.weak = typeDefenses.weak.filter(
        (value) => !tempWeak.includes(value)
    );
    typeDefenses.very_weak.push(...tempWeak);

    typeDefenses.resistant = typeDefenses.resistant.filter(
        (value) => !typeDefenses.weak.includes(value)
    );

    let tempResistant: string[] = Object.keys(
        findDuplicateElements(typeDefenses.resistant)
    );
    typeDefenses.resistant = typeDefenses.resistant.filter(
        (value) => !tempResistant.includes(value)
    );
